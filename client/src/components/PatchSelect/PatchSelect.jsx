    ...
    try {
      ...
      const fetchedData = await Promise.all(
        Array.from({ length: end - start + 1 }, (_, index) =>
          fetch(
            `${baseUrl}/performance/${championName}/stats/${start + index}`
      ...
      if (compareChampion) {
        const compareData = await Promise.all(
          Array.from({ length: end - start + 1 }, (_, index) =>
            fetch(
              `${baseUrl}/performance/${compareChampion}/stats/${start + index}`
            ).then((response) => response.json())
          )
        );
        ...
        <div className="chart-row">
          ...
          <div className="Win-chart">
            <WinRateChart
              ...
              compareChampion={compareChampion}
            />
          </div>
        </div>
        <div className="chart-row">
            ...
            <div className="image-container">
              {Object.keys(championData).map((patch, index) => (
                <React.Fragment key={patch}>
                  ...
                  />
                  {(index + 1) % 3 === 0 && <br />}{" "}
                  {/* Add a line break after every third image */}
                </React.Fragment>
              ))}
            </div>
          </div>
