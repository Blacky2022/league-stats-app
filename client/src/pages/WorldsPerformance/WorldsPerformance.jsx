...
export function WorldsPerformance() {
	...
	return (
		<AuthProvider>
			<Navbar />
			<div>
				<h1 className='title'>Postacie Worlds - Ilość rozegranych gier: {totalGames}</h1>
				<div className='table_container'>
					{champions.length === 0 ? (
						...
						<table className='champions-table'>
							<thead>
								<tr>
									<th onClick={() => handleSort('champion')}>Champion {sortIndicator('champion')}</th>
									<th onClick={() => handleSort('sum_total')}>Sum Total {sortIndicator('sum_total')}</th>
									<th onClick={() => handleSort('win_total')}>Win Total {sortIndicator('win_total')}</th>
									<th onClick={() => handleSort('sum_bans')}>Sum Bans {sortIndicator('sum_bans')}</th>
									<th onClick={() => handleSort('sum_pick_ban')}>Sum Pick Ban {sortIndicator('sum_pick_ban')}</th>
									<th onClick={() => handleSort('win_rate')}>Win Rate {sortIndicator('win_rate')}</th>
									<th onClick={() => handleSort('pick_rate')}>Pick Rate {sortIndicator('pick_rate')}</th>
									<th onClick={() => handleSort('ban_rate')}>Ban Rate {sortIndicator('ban_rate')}</th>
									<th onClick={() => handleSort('pick_ban_rate')}>Pick Ban Rate {sortIndicator('pick_ban_rate')}</th>
								</tr>
							</thead>
							<tbody>
