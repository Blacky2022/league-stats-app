...
export function WorldsPerformance() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/worlds/champions', {
      method: 'GET',
      headers: {
  ...
  const sortedChampions = [...champions].sort((a, b) => {
	if (sortColumn === 'champion') {
	  ...
	  const aValue = sortColumn === 'win_rate' ? calculateWinRate(a.win_total, a.sum_total) :
		sortColumn === 'pick_rate' ? calculatePickRate(a.sum_pick_ban, a.sum_bans) :
		sortColumn === 'ban_rate' ? calculateBanRate(a.sum_bans) :
		sortColumn === 'pick_ban_rate' ? calculatePickBanRate(a.sum_pick_ban) : a[sortColumn];
	  const bValue = sortColumn === 'win_rate' ? calculateWinRate(b.win_total, b.sum_total) :
		sortColumn === 'pick_rate' ? calculatePickRate(b.sum_pick_ban, b.sum_bans) :
		sortColumn === 'ban_rate' ? calculateBanRate(b.sum_bans) :
		sortColumn === 'pick_ban_rate' ? calculatePickBanRate(b.sum_pick_ban) : b[sortColumn];
